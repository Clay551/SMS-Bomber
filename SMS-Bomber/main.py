...
Discord : https://discord.gg/hKckh49p 
    ...
    while True:
        ...
        try:
            ...
            print()
            choice = Functions.get_input(f"{Fore.CYAN}[=]{Col.dark_gray} Enter Your Choice: {Col.green}", lambda x: x in [str(i) for i in choices])
            number = Functions.get_input(f"{Fore.CYAN}[=]{Col.dark_gray} Enter the phone number {Fore.CYAN}[9xxxxxxxxx]{Col.gray}: {Col.green}", checker=lambda x: x != "" and x.isnumeric() and x.startswith("9") and len(x) == 10)
